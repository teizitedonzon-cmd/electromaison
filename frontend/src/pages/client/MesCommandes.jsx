...
const ICONES_STATUT = { en_attente:'⏳', confirmee:'✅', en_livraison:'🚚', livree:'🎉', annulee:'❌' };
          ...
          <div key={c._id} style={styles.card}>
            <div style={styles.cardHeader}>
              ...
              <span style={{ ...styles.statutBadge, ...COULEURS[c.statut] }}>
                {ICONES_STATUT[c.statut]} {c.statut.replace('_', ' ')}
              </span>
            </div>
            <div style={styles.cardBody}>
              <span>{c.lignes?.length} article(s)</span>
              <span style={styles.montant}>{c.montantTotal.toLocaleString('fr-FR')} FCFA</span>
            </div>
            <button onClick={() => setDetail(c)} style={styles.btnDetail}>Voir le détail</button>
          </div>
        ))}
      </div>
        ...
        <div style={styles.overlay} onClick={() => setDetail(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            {detail.lignes?.map((l,i) => (
              <div key={i} style={{display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid #eee', fontSize:'0.9rem'}}>
                <span>{l.nomProduit} × {l.quantite}</span>
                <span>{l.sousTotal.toLocaleString('fr-FR')} FCFA</span>
              </div>
            ))}
            <div style={{fontWeight:'800', textAlign:'right', marginTop:'12px', color:'#C8410A', fontSize:'1.1rem'}}>
              Total : {detail.montantTotal.toLocaleString('fr-FR')} FCFA
            </div>
...
const styles = {
  ...
  statutBadge: { padding:'6px 14px', borderRadius:'50px', fontSize:'0.8rem', fontWeight:'700' },
