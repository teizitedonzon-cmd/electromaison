    ...
    <div style={styles.layout} className="admin-responsive-layout">
      ...
      <main style={styles.main} className="admin-responsive-main">
        ...
        <div style={styles.filtres}>
          <button onClick={() => setFiltreStatut('')} style={{ ...styles.filtrBtn, ...(filtreStatut === '' ? styles.filtrActif : {}) }}>Toutes</button>
          {STATUTS.map((s) => (
            <button key={s} onClick={() => setFiltreStatut(s)} style={{ ...styles.filtrBtn, ...(filtreStatut === s ? styles.filtrActif : {}) }}>
              {s.replace('_', ' ')}
            </button>
          ))}
        </div>
          ...
          <table style={styles.table}>
            <thead>
              <tr style={styles.thead}>
                {['Ref.', 'Client', 'Montant', 'Statut', 'Paiement', 'Date', 'Actions'].map((h) => <th key={h} style={styles.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {commandes.map((c) => (
                <tr key={c._id} style={styles.tr}>
                  ...
                  <td style={styles.td}><strong>{c.client?.prenom} {c.client?.nom}</strong><br /><small style={{ color: '#888' }}>{c.client?.telephone}</small></td>
                  <td style={styles.td}><strong>{Number(c.montantTotal).toLocaleString('fr-FR')} FCFA</strong></td>
                  <td style={styles.td}>
                    <select value={c.statut} onChange={(e) => changerStatut(c._id, e.target.value)} style={{ ...styles.statutSelect, ...COULEURS_STATUT[c.statut] }}>
                  ...
                  <td style={styles.td}><button onClick={() => setDetail(c)} style={styles.btnDetail}><Icon name="eye" size={15} /> Detail</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {detail && (
          <div style={styles.overlay} onClick={() => setDetail(null)}>
            <div style={styles.modal} className="responsive-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div style={styles.totalDetail}>Total : {Number(detail.montantTotal).toLocaleString('fr-FR')} FCFA</div>
...
const styles = {
  ...
  btnDetail: { display: 'inline-flex', alignItems: 'center', gap: 6, background: '#EBF5FB', color: '#2980B9', border: 'none', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.82rem' },
